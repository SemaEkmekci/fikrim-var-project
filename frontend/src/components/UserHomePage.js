...
export default class UserHomePage extends Component {
  ...
  handleSubmit = (e) => {
    e.preventDefault();
    const { unit, projectName, projectPurpose, projectDetails, image, status } =
      this.state;
    if (unit == "") {
      Swal.fire({
    ...
    fetch("http://localhost:5000/projectInfo", {
      ...
      .then((data) => {
        ...
        if (data.status === "ok") {
          ...
          Swal.fire({
            text: 'Proje Gönderme İşlemi Sırasında Bir Hata Oluştu. Daha Sonra Tekrar Deneyin',
            icon: 'error',
            confirmButtonColor: '#3085d6', 
          });
      ...
      .catch((error) => {
        console.error(
          "Proje gönderme işlemi sırasında bir hata oluştu:",
          error
        );
        Swal.fire({
      ...
      .then((data) => {
        ...
        if (data.status === "ok") {
          if(data.data === "true"){
            ...
            Swal.fire({
              text: 'E-Posta Başkası Tarafından Kullanılıyor.',
              icon: 'error',
              confirmButtonColor: '#3085d6', 
            });
          }
          ...
          Swal.fire({
            text: 'E-posta başkası tarafıdan kullanılıyor',
            icon: 'error',
            confirmButtonColor: '#3085d6', 
          });
                ...
                <Col lg={12}>
                  <div>
                    <textarea
                      ...
                      onChange={(e) =>
                        this.setState({ projectPurpose: e.target.value })
                      }
                      required
                    />
                  </div>
                ...
                <Col lg={12}>
                  <div>
                    <textarea
                      ...
                      onChange={(e) =>
                        this.setState({ projectDetails: e.target.value })
                      }
                      required
                    />
                  </div>
          ...
          <div className="menu-bar">
            <div className="menu">
              <li className="nav-link">
                <a className="text nav-text button" href="Application">
                  Başvurularım
                </a>
              </li>
            </div>
